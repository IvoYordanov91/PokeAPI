  ...
  pokedex$$.addEventListener("click", () => {
    ...
    for (let i = 0; i < pokePs$$.length; i++) {
        if (pokePs$$[i].innerHTML.indexOf(",") !== -1) {
            ...
            <p class="${pokePs$$[i].innerText.split(",")[0]} card-subtitle multiple-type">${pokePs$$[i].innerText.split(",")[0]}</p>
            <p class="${pokePs$$[i].innerText.split(",")[1]} card-subtitle multiple-type">${pokePs$$[i].innerText.split(",")[1]}</p>
            </div>
            `;            
        }
    }
